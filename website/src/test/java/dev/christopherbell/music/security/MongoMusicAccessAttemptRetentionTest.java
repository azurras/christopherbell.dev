...
class MongoMusicAccessAttemptRetentionTest {
  @Test
  void refreshedExpiryIsRecheckedAfterBoundedSelection() {
    ...
    KindScopedMongoOperations<MusicAccessAttempt> operations =
        mock(KindScopedMongoOperations.class);
    when(factory.forType(MusicAccessAttempt.class)).thenReturn(operations);
    var storedExpiry = new AtomicReference<>(cutoff.minusSeconds(1));
    when(operations.find(any(Query.class), eq(Pageable.unpaged())))
        .thenAnswer(invocation -> {
          var selected = new MusicAccessAttempt(
            "race", MusicAccessPrincipalType.IP, "127.0.0.1", "denied", 1,
            cutoff.minusSeconds(2), cutoff.minusSeconds(2), storedExpiry.get());
    ...
    assertThat(new MongoMusicAccessAttemptRepository(factory).deleteExpired(cutoff, 1)).isZero();
