...
class PersistenceProfileConfigurationTest {
  ...
  void localAndTestProfilesSelectPostgresqlDatabaseTest() throws IOException {
    for (String resourceName : List.of("application-local.yml", "application-test.yml")) {
      ...
      assertThat(source.getProperty("spring.datasource.url").toString()).contains("/test");
      assertThat(source.getProperty("spring.autoconfigure.exclude[0]").toString())
          .contains("MongoAutoConfiguration");
      assertThat(source.getProperty("spring.autoconfigure.exclude[1]").toString())
          .contains("DataMongoRepositoriesAutoConfiguration");
    }
  }
