...
public class PostgresRandomVinImportStateRepository implements RandomVinImportStateRepository {
  ...
  @Override public Optional<RandomVinImportState> findById(String id) {
    return database.selectFrom(RANDOM_VIN_IMPORT_STATE)
        .where(RANDOM_VIN_IMPORT_STATE.IMPORT_STATE_ID.eq(id)).fetchOptional(row -> {
          var robots = Boolean.TRUE.equals(row.getRobotsPolicyPresent())
  ...
  @Override public RandomVinImportState save(RandomVinImportState state) {
    ...
    var insert = database.insertInto(RANDOM_VIN_IMPORT_STATE)
        ...
        .set(RANDOM_VIN_IMPORT_STATE.CALLS_TODAY, state.getCallsToday())
        ...
        .set(RANDOM_VIN_IMPORT_STATE.LIFETIME_CALLS, state.getLifetimeCalls())
        .set(RANDOM_VIN_IMPORT_STATE.LIFETIME_VINS_PROCESSED, state.getLifetimeVinsProcessed())
        .set(RANDOM_VIN_IMPORT_STATE.NOTES, state.getNotes())
        .set(RANDOM_VIN_IMPORT_STATE.PERMANENTLY_DISABLED, state.getPermanentlyDisabled())
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_ALLOWED, robots == null ? null : robots.getAllowed())
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_CHECKED_ON, robots == null ? null : offset(robots.getCheckedOn()))
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_FAIL_CLOSED, robots == null ? null : robots.getFailClosed())
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_POLICY_PRESENT, robots != null)
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_REASON, robots == null ? null : robots.getReason())
        .set(RANDOM_VIN_IMPORT_STATE.VINS_PROCESSED_TODAY, state.getVinsProcessedToday());
    insert.onConflict(RANDOM_VIN_IMPORT_STATE.IMPORT_STATE_ID).doUpdate()
        ...
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_POLICY_PRESENT, robots != null)
        .set(RANDOM_VIN_IMPORT_STATE.ROBOTS_REASON, robots == null ? null : robots.getReason())
        .set(RANDOM_VIN_IMPORT_STATE.VINS_PROCESSED_TODAY, state.getVinsProcessedToday()).execute();
