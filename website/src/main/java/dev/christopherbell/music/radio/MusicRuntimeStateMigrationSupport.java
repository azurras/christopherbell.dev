...
public final class MusicRuntimeStateMigrationSupport implements MusicRuntimeStateMigrationPort {
  ...
  @Override
  public PreparedMigration prepare(
      List<Document> queueSources, List<Document> radioSources) {
    return new Prepared(sources(queueSources, radioSources));
  }

  private static Sources sources(
      List<Document> queueDocuments, List<Document> radioDocuments) {
    var queue = optionalSingleton(queueDocuments)
        .map(MusicRuntimeStateMigrationSupport::queueSource);
    var radio = optionalSingleton(radioDocuments)
        .map(MusicRuntimeStateMigrationSupport::radioSource);
    return new Sources(queue, radio);
  }

  private static Optional<Document> optionalSingleton(List<Document> documents) {
    if (documents.size() > 1) {
  ...
  private static void requireEquivalent(List<Document> documents, Sources sources) {
    int expectedSize = (sources.queue().isPresent() ? 1 : 0)
        + (sources.radio().isPresent() ? 1 : 0);
  ...
  private record Prepared(Sources sources) implements PreparedMigration {
    @Override
    public List<Document> documentsToInsert(List<Document> existingTargets) {
      if (!existingTargets.isEmpty()) {
        MusicRuntimeStateMigrationSupport.requireEquivalent(existingTargets, sources);
        return List.of();
      }

      var expected = new ArrayList<Document>(2);
      sources.queue().map(MusicRuntimeStateMigrationSupport::queueTarget).ifPresent(expected::add);
      sources.radio().map(MusicRuntimeStateMigrationSupport::radioTarget).ifPresent(expected::add);
      return List.copyOf(expected);
    }

    @Override
    public void requireEquivalent(List<Document> targetDocuments) {
      MusicRuntimeStateMigrationSupport.requireEquivalent(targetDocuments, sources);
    }
  }
}
