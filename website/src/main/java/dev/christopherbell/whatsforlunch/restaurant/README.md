...
- `RestaurantImportState` stores the last completed import time and month. On application startup, WFL checks whether the next configured monthly schedule after the last successful import is due; if so, it runs a catch-up import immediately. With no successful import history, startup runs the existing first-import catch-up.
